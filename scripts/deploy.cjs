...
async function main() {
  ...
  console.log("\n1. Deploying ActionHub...");
  const ActionHub = await ethersHelper.getContractFactory("ActionHub");
  const actionHub = await deployWithNonce(ActionHub);
  console.log("ActionHub deployed to:", await actionHub.getAddress());
  ...
  const DEXAggregator = await ethersHelper.getContractFactory("DEXAggregator");
  const dexAggregator = await deployWithNonce(DEXAggregator);
  console.log("DEXAggregator deployed to:", await dexAggregator.getAddress());

  console.log("\n4. Deploying YieldFarmer...");
  const YieldFarmer = await ethersHelper.getContractFactory("YieldFarmer");
  const yieldFarmer = await deployWithNonce(YieldFarmer);
  console.log("YieldFarmer deployed to:", await yieldFarmer.getAddress());

  console.log("\n5. Deploying ChainlinkAnalyzer...");
  const ChainlinkAnalyzer = await ethersHelper.getContractFactory("ChainlinkAnalyzer");
  const chainlinkAnalyzer = await deployWithNonce(ChainlinkAnalyzer);
  console.log("ChainlinkAnalyzer deployed to:", await chainlinkAnalyzer.getAddress());
  ...
  console.log("\n=== Deployment Summary ===");
  console.log("ActionHub:", await actionHub.getAddress());
  console.log("MNTTransferExecutor:", await mntExecutor.getAddress());
  console.log("DEXAggregator:", await dexAggregator.getAddress());
  console.log("YieldFarmer:", await yieldFarmer.getAddress());
  console.log("ChainlinkAnalyzer:", await chainlinkAnalyzer.getAddress());

  console.log("\n=== Configuration Status ===");
  console.log("✓ All contracts deployed with latest fixes");
